...
/**
 * <p>
 * Renders an HTML &lt;input&gt; element of type <code>checkbox</code>, whose "checked" status is initialized based on
 * whether the specified value matches one of the elements of the underlying property's array of current values. This
 * element is useful when you have large numbers of checkboxes, and prefer to combine the values into a single
 * array-valued property instead of multiple boolean properties. This tag is only valid when nested inside a form tag
 * body.
 * </p>
 * <p>
 * <strong>WARNING</strong>: In order to correctly recognize cases where none of the associated checkboxes are selected,
 * the <code>ActionForm</code> bean associated with this form must include a statement setting the corresponding array
 * to zero length in the <code>reset()</code> method.
 * </p>
 * <p>
 * The value to be returned to the server, if this checkbox is selected, must be defined by one of the following
 * methods:
 * </p>
 * <ul>
