...
/**
 * Allows to determine possible values for an argument annotated with {@link TrustAssertion}.
 *
...
public interface Options<T> {
	...
	/**
	 * Gets possible values for an argument annotated with {@link TrustAssertion}. The default implementation {@link StringOptions} just
	 * passes on a given String array as possible values. Sometimes the possible values are more dynamic and depend on the context.
	 * Therefore, an Options implementation can also determine possible values based on another argument to the same call. The example below
	 * shows how to get possible options from a custom DetailOptions implementation which needs a personId for that:
	 * 
	 * <pre>
	 ...
	 * order. In the example above, DetailOptions receives the personId and can read possible options for that particular person.
	 * </p>
	 *
