...
public class RequestContextFactoryImpl implements RequestContextFactory {
	...
	private RequestContextHolder save(final RequestInitializer requestInitializer, final RequestContext holder) {
		requestInitializer.initRequest(holder);

		RequestWrapper requestWrapper = requestInitializer.createRequestWrapper(holder);
		ResponseWrapper responseWrapper = requestInitializer.createResponseWrapper(holder);

		holder.update(requestWrapper, responseWrapper);
		return holder;
	}
