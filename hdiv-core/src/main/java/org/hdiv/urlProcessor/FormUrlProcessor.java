...
public class FormUrlProcessor extends AbstractUrlProcessor {
	...
	public String processUrl(final HttpServletRequest request, String url, Method method) {
		...
		final UrlData urlData = createUrlData(url, method, request);
		if (isHdivStateNecessary(urlData)) {
			...
			// Process url params
			final String processedParams = dataComposer.composeParams(urlData.getUrlParams(), method.toString(), Constants.ENCODING_UTF_8);
